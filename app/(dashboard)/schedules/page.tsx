...
import {
  ...
  Trash2, Loader2, Home, RefreshCw, Clock, FileDown,
} from "lucide-react";
...
function MinistryCard({ ministryName, ministryColor, entries, onRemove }: {
  ministryName: string; ministryColor: string; entries: ScheduleEntry[]; onRemove: (id: string) => void;
}) {
  return (
      ...
      <div className="space-y-1.5">
        {entries.map((entry) => (
          <div key={entry.id} className="flex items-center gap-2 group">
            ...
            <button onClick={() => onRemove(entry.id)} className="p-1 rounded text-white/10 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100">
...
function CultoSection({ culto, entriesByMinistry, ministries, onRemove }: {
  culto: CultoSlot;
    ...
    <div className="card p-4">
      ...
      {Object.keys(entriesByMinistry).length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
          {ministries.filter((m) => entriesByMinistry[m.id]).map((m) => (
            <MinistryCard key={m.id} ministryName={m.name} ministryColor={m.color} entries={entriesByMinistry[m.id]} onRemove={onRemove} />
          ))}
        </div>
      )}
    </div>
...
function AddEntryForm({ ministries, volunteers, onAdd, onClose }: {
  ministries: { id: string; name: string; color: string }[];
  ...
  async function handleAdd() {
    if (!ministryId || !volunteerId) { toast.error("Selecione o ministério e o voluntário."); return; }
    setSaving(true);
    try { await onAdd({ culto, ministryId, volunteerId, role: role || undefined }); onClose(); }
    ...
    <div className="space-y-4">
      <FormField label="Culto" required>
        <div className="grid grid-cols-3 gap-2">
          {CULTOS.map((c) => (
            <button key={c} type="button" onClick={() => setCulto(c)}
              className={cn("py-2.5 rounded-xl text-[13px] font-medium border transition-all",
                culto === c ? "bg-brand-500/10 border-brand-500/40 text-brand-300" : "border-surface-400/40 text-white/40 hover:border-surface-400 hover:text-white/70"
              )}>
              {CULTO_LABELS[c]}
            </button>
          ))}
      ...
      <FormField label="Ministério" required>
        <select value={ministryId} onChange={(e) => setMinistryId(e.target.value)} className="input-base appearance-none">
          <option value="">Selecione um ministério...</option>
          {ministries.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}
        </select>
      </FormField>
      <FormField label="Voluntário" required>
        <select value={volunteerId} onChange={(e) => setVolunteerId(e.target.value)} className="input-base appearance-none">
          <option value="">Selecione um voluntário...</option>
          {volunteers.length === 0 && <option disabled>Nenhum voluntário ativo cadastrado</option>}
          {volunteers.map((v) => <option key={v.id} value={v.id}>{v.name}</option>)}
        </select>
      </FormField>
      <FormField label="Função (opcional)" hint="Ex: Baterista, Câmera, Recepcionista">
        <input value={role} onChange={(e) => setRole(e.target.value)} placeholder="Função na escala..." className="input-base" />
      </FormField>
      <div className="flex justify-end gap-3 pt-2">
        <ModalCancelButton onClick={onClose} />
        <button onClick={handleAdd} disabled={saving || !ministryId || !volunteerId} className="btn-primary px-5">
          {saving ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Adicionando...</> : "Adicionar à escala"}
        </button>
      </div>
    </div>
  );
...
export default function SchedulesPage() {
  ...
  const { schedule, entries, loading, entriesByCulto, referenceDate, goNextWeek, goPrevWeek, goToday, addEntry, removeEntry, refetch } = useSchedules();
    ...
    <div className="p-5 sm:p-6 space-y-5 page-enter">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Link
            href="/schedules/export"
            className="flex items-center gap-2 px-4 py-2 rounded-xl border border-white/20 text-white/60 hover:text-white hover:border-white/40 transition-all text-[13px] font-medium"
          >
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {CULTOS.map((culto) => (
            <CultoSection key={culto} culto={culto} entriesByMinistry={entriesByCulto[culto] ?? {}} ministries={ministriesForForm} onRemove={removeEntry} />
          ))}
        </div>
      )}

      <Modal open={modal} onClose={() => setModal(false)} title="Escalar voluntário" subtitle={schedule ? sundayFullLabel(referenceDate) : ""} size="md">
        <AddEntryForm ministries={ministriesForForm} volunteers={volunteersForForm} onAdd={handleAdd} onClose={() => setModal(false)} />
      </Modal>
    </div>
  );
}